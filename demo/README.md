...
`logic_coloc_demo.mp4` —— 线上站点的完整走查，2 分 10 秒，860×1864，30fps，无音轨。
...
录屏时浏览器视口是 430×932（1× DPR），成片放大到 860×1864 输出，所以是「2× DPR 的观感」。
...
原始 webm 没有进仓库（14MB 的二进制放 git 里会一直跟着 clone 走）。
它挂在 fork 的 Release 上（本仓库还没有这枚 Release），从
<https://github.com/qgeng1465/isoLogic/releases/tag/demo-recording> 下载
`raw-demo-recording.webm`，放到本目录，然后：
...
| `HEAD_TRIM` | 片头掐掉的秒数，默认 `3.9`。见下方坑 1：腾讯云测试域名的「风险提醒」页会被录进片头，不掐掉的话成片第一帧和海报都是那张警告页。换到没有这道闸的域名后改成 `0` |
...
- 流程里点的是**真实按钮**，没有 mock；两处 LLM 调用是真调后端，所以单次录制
  约 4 分半（原始 webm 4 分 35 秒，其中 ~2.7 分钟在等模型）。
...
1. **CloudBase 测试域名有人工确认页**。首次访问会先落到「风险提醒」页，
   ...
   ⚠️ 但**这几秒会被录进原始素材的开头**（警告页 + 3 秒倒计时）——成片靠 `edit_demo.py`
   的 `HEAD_TRIM` 掐掉，海报帧也是从那之后取的。不掐的话，观众第一眼看到的是腾讯云那句
   「仅供开发测试使用…请勿泄露个人信息」，演示效果就毁了。
