...
| 4️⃣ **演示视频**（加分，2 分 10 秒完整走查） | **在线播放：<https://qgeng1465.github.io/isoLogic/>**（点开即看，含走查路线）｜ 仓库内 [`demo/logic_coloc_demo.mp4`](demo/logic_coloc_demo.mp4)（860×1864 竖屏，无音轨）；剪辑方法与原始素材见 [`demo/README.md`](demo/README.md) ｜ 知乎（待上传，占位：`<知乎视频链接>`） |
...
所以目录名必须和包名一致。而 GitHub 上的仓库名是 `isoLogic`（大写 I、驼峰），
...
（镜像标签只能小写，所以这里保留 `logic-coloc`，与仓库名 `isoLogic` 不同是正常的。）
...
演示视频可以自己重剪：**原始 webm（4 分 35 秒全流程，无加速）放在 fork 的 Release 里**——
<https://github.com/qgeng1465/isoLogic/releases/tag/demo-recording>，下载后改
